...
const DedicatedBanner = memo(props => {
  const { values, onChange } = props;

  const styles = dedicatedBannerStyles();
  ...
  const handleIconChange = useCallback(e => onChange('banner_icon', e.target.value), [onChange]);

  const handleStyleChange = useCallback(e => onChange('banner_style', e.target.value), [onChange]);

  const handleMessageChange = useCallback(val => onChange('banner_message', val), [onChange]);
  ...
  return (
    <Box sx={styles.root}>
      ...
      >
        Enable dedicated banner to communicate important notifications across the platform, including system
        warnings, maintenance alerts, policy updates, and general informational messages.
      </Typography>
        ...
        <Box sx={styles.cardRow}>
          <Box sx={styles.cardLabel}>
            ...
            <Typography
              ...
              sx={styles.cardHint}
            >
              When enabled, users can close the banner. Dismissed state persists until new banner is
              configured or user logs out / clears session cache
            </Typography>
          </Box>
          <Switch
            checked={dismissible}
      ...
      <Box sx={styles.editorCard}>
        <Box sx={styles.editorHeader}>
          <Typography
            variant="body2"
            sx={styles.editorTitle}
          >
            Banner Message
          </Typography>
          <Typography
            variant="caption"
            sx={styles.editorHint}
          >
            Fill in the message to be shown in the banner. Supports Markdown formatting.
          </Typography>
        </Box>
