...
import { Box, Button, Chip, Drawer, IconButton, Tab, Tabs, Typography } from '@mui/material';
...
const ProjectActivityDrawer = memo(props => {
  const { open, onClose, project } = props;

  const styles = projectActivityDrawerStyles();
  ...
  const {
    ...
    skip: !open || !projectId || viewMode !== 'spans',
  });
  ...
  const {
    ...
    refetchOnMountOrArgChange: true,
    skip: !open || !projectId || viewMode !== 'traces',
  });
...
ProjectActivityDrawer.displayName = 'ProjectActivityDrawer';

/** @type {MuiSx} */
const projectActivityDrawerStyles = () => ({
  drawer: {
    '& .MuiDrawer-paper': {
      width: '75vw',
      maxWidth: '75vw',
