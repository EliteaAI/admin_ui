...
import { Alert, Box, Button, Chip, CircularProgress, Collapse, Snackbar, Typography } from '@mui/material';
...
import { ALWAYS_SHOW_PLUGIN_UPDATE } from '@/helpers/env.helpers';

import PluginConfigDrawer from './components/PluginConfigDrawer';
import PylonLogsDrawer from './components/PylonLogsDrawer';
...
const pluginKey = (pylonId, name) => {
  return `${pylonId}::${name}`;
};

const AdvancedSection = memo(() => {
  const styles = advancedSectionStyles();
              ...
              {plugins.map(plugin => {
                ...
                return (
                  ...
                  >
                    ...
                    <Typography
                      variant="body2"
                      sx={[styles.cell, hasUpdate && styles.versionHighlight]}
                    >
                      {ps.repoVersion || '-'}
                    </Typography>
                    <Box sx={styles.cell}>
...
const advancedSectionStyles = () => ({
  ...
  statusDot:
    ...
    ({ palette }) => ({
      ...
      height: '0.5rem',
      borderRadius: '50%',
      backgroundColor: active ? palette.status.active : palette.status.inactive,
    }),
  actionsCell: {
    display: 'flex',
    alignItems: 'center',
