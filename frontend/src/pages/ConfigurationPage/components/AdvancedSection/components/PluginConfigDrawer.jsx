...
import { yamlLinter } from '@/pages/ConfigurationPage/components/AdvancedSection/helpers/yamlLinter.helpers';
import { yaml } from '@codemirror/lang-yaml';
import { lintGutter } from '@codemirror/lint';
import { EditorView } from '@codemirror/view';
import CodeMirror from '@uiw/react-codemirror';

const PluginConfigDrawer = memo(props => {
  const { open, plugin, onClose } = props;

  const styles = pluginConfigDrawerStyles();
  ...
  const pluginId = plugin ? (plugin.name ? `${plugin.pylon_id}:${plugin.name}` : plugin.pylon_id) : '';
  ...
  const rawExtensions = useMemo(() => [yaml(), EditorView.lineWrapping, yamlLinter, lintGutter()], []);
  const readOnlyExtensions = useMemo(() => [yaml(), EditorView.lineWrapping], []);
...
PluginConfigDrawer.displayName = 'PluginConfigDrawer';

/** @type {MuiSx} */
const pluginConfigDrawerStyles = () => ({
  drawer: {
    '& .MuiDrawer-paper': {
      width: '50vw',
      maxWidth: '50vw',
