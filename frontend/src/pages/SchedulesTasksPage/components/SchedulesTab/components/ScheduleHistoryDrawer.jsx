...
const ScheduleHistoryDrawer = memo(props => {
  const { open, onClose, schedule } = props;

  const styles = scheduleHistoryDrawerStyles();
      ...
      <Box sx={styles.root}>
        ...
        <Box sx={styles.listContainer}>
          ...
          {rows.map((row, idx) => (
            <Box
              key={row.span_id || idx}
              sx={styles.executionRow}
            >
              <Box sx={styles.executionIcon}>
                {row.is_error ? (
                  <ErrorOutline sx={styles.statusIcon('error')} />
                ) : (
                  <CheckCircleOutline sx={styles.statusIcon('success')} />
                )}
              </Box>
...
ScheduleHistoryDrawer.displayName = 'ScheduleHistoryDrawer';

/** @type {MuiSx} */
const scheduleHistoryDrawerStyles = () => ({
  drawer: {
    '& .MuiDrawer-paper': {
      width: '36rem',
      maxWidth: '90vw',
