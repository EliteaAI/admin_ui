...
import PermissionGroupRow from './components/PermissionGroupRow';
import PermissionRow from './components/PermissionRow';

const PermissionMatrix = memo(props => {
  const { rows, roles, search, onChange, readOnly, disabledRoles = [] } = props;

  const styles = permissionMatrixStyles();
  ...
  useEffect(() => {
    if (search) {
      const groupsWithMatches = new Set(grouped.map(([groupName]) => groupName));
  ...
  const handlePermissionChange = useCallback(
    (permName, role, value) => {
      onChange(draft => draft.map(r => (r.name === permName ? { ...r, [role]: value } : r)));
    },
    [onChange],
  );
  ...
  const handleGroupToggle = useCallback(
    (groupName, role, value) => {
      ...
      onChange(draft => draft.map(r => (permNames.has(r.name) ? { ...r, [role]: value } : r)));
    },
    [grouped, onChange],
  );
  ...
  return (
    <Box sx={styles.root(gridColumns)}>
