...
const LiteLLMPage = memo(() => {
  ...
  const litellmSection = useMemo(() => schemasData?.sections?.find(s => s.id === 'litellm'), [schemasData]);
  ...
  const canEdit = useMemo(() => hasPermission(PERMISSIONS.litellm.edit), [hasPermission]);
      ...
      <DrawerPage>
        ...
        <Box sx={styles.emptyContainer}>
          <Typography
            ...
            color="text.metrics"
          >
            LiteLLM configuration is not available. The runtime_engine_litellm plugin may not be running.
          </Typography>
        </Box>
      </DrawerPage>
    );
