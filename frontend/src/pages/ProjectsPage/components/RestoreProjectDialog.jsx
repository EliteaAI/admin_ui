...
import { useProjectRestoreMutation } from '@/api/projectBackup.api';
...
const RestoreProjectDialog = memo(props => {
  ...
  const styles = restoreProjectDialogStyles();
  ...
  const canFull = useMemo(() => hasPermission(PERMISSIONS.projects.restore.full), [hasPermission]);
  ...
  const isMismatch =
    typeof artifact?.project_id === 'number' && project?.id != null && artifact.project_id !== project.id;
    ...
    >
      ...
      <DialogContent>
        <Typography
          ...
          sx={styles.gapBelow}
        >
          Restore a backup into project <strong>{project?.name}</strong> (schema <code>p_{project?.id}</code>
          ).
        </Typography>
        ...
        <Box sx={styles.options}>
          ...
          {!canFull && (
            <Typography
              ...
              color="text.secondary"
            >
              Requires the <code>projects.projects.restore.full</code> permission. Without it only safe
              (redacted) backups can be restored.
            </Typography>
          )}
        </Box>
        ...
        {fullMode && (
          ...
          >
            The uploaded file is piped to <code>psql</code> as-is (DDL included), cannot be partially
            restored, and will be rejected if it is not a raw <code>pg_dump</code>.
          </Alert>
        )}
        ...
        {artifact?.kind && (
          <Box
            component="div"
            sx={styles.artifactMeta}
          >
            {artifact.kind === 'pg_dump' ? 'raw pg_dump' : 'redacted backup'}
            {' · source: '}
            {artifact.project_name ?? '-'}
            {artifact.project_id !== undefined ? ` (project ${artifact.project_id})` : ''}
        ...
        {isMismatch && (
          <>
            <Alert
              ...
              sx={styles.gapBelowSmall}
            >
              This backup was taken from project {artifact.project_id}, not {project?.id}. Restoring it here
              mixes data between projects.
            </Alert>
            <FormControlLabel
              control={
                <Checkbox
        ...
        {truncate && !fullMode && (
          ...
          >
            ...
            <code>CASCADE</code> removes rows referencing them. Without this option rows are merged (
...
/** @type {MuiSx} */
const restoreProjectDialogStyles = () => ({
  filePicker: {
    display: 'flex',
    alignItems: 'center',
    gap: '0.75rem',
