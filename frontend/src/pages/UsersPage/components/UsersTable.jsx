...
import { Box, Chip, IconButton, MenuItem, Select, Skeleton, Tooltip, Typography } from '@mui/material';
...
import { ADMIN_ROLES } from '@/constants/permissions.constants';
...
const UsersTable = memo(props => {
  ...
  const styles = useMemo(() => usersTableStyles(), []);
  ...
  const renderCell = useCallback(
    ...
    [styles, onSetAdminRole, isSuperAdmin, isAdmin],
  );
  ...
  const renderActions = useCallback(
    ...
    [styles, onDelete, onSuspend, onActivity],
  );
...
const usersTableStyles = () => ({
  ...
  suspendedRow: ({ palette }) => ({
    opacity: 0.5,
    backgroundColor: palette.background.subtle.default,
  }),
  roleSelect: {
    minWidth: '7rem',
    '& .MuiSelect-select': {
