...
const warningPctError = value => {
  if (value === '' || value === null || value === undefined) return 'Required (1-100)';
...
const CostBudgets = memo(props => {
  ...
  const styles = costBudgetsStyles();
  ...
  const handleModeChange = useCallback(e => onChange('usage_mode', e.target.value), [onChange]);
  ...
  return (
    <Box sx={styles.root}>
      ...
      >
        Limit monthly spend on shared models per project and per user. Limits are enforced before each call,
        so an over-budget project is blocked rather than billed. Individual limits are set on the Budgets
        page; the values here apply only where nothing has been set explicitly.
      </Typography>
          ...
          <Box sx={styles.cardRow}>
            <Box sx={styles.cardLabel}>
              ...
              <Typography
                ...
                sx={styles.cardHint}
              >
                Apply the limits below to projects and users with no limit set explicitly. When disabled,
                anything without an explicit limit stays unlimited.
              </Typography>
            </Box>
            <Switch
