...
import { formatPerMillion } from '@/pages/ModelPricesPage/helpers/modelPrices.helpers';
...
const ModelPricesTable = memo(props => {
  const {
    rows = [],
    total = 0,
    page = 0,
  ...
  const styles = useMemo(() => modelPricesTableStyles(), []);
  ...
  const renderCell = useCallback(
    (column, value, row) => {
      ...
      if (column.field === 'input_cost_per_token' || column.field === 'output_cost_per_token') {
        return (
          <Typography
            variant="bodyMedium"
            sx={styles.cellText}
...
ModelPricesTable.displayName = 'ModelPricesTable';

/** @type {MuiSx} */
const modelPricesTableStyles = () => ({
  tableContainer: {
    height: '100%',
    width: '100%',
    display: 'flex',
