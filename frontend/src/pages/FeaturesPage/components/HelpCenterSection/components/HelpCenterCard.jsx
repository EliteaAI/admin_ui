...
import ResourceVersionRow from './ResourceVersionRow';

const HelpCenterCard = memo(props => {
  const { card, values, onChange, systemInfo } = props;

  const styles = helpCenterCardStyles();
      ...
      {versionRows.length > 0 && (
        <Box sx={styles.versionCard}>
          ...
          <Box sx={styles.versionContent}>
            ...
            {versionRows.some(r => !r.valueKey) && <Divider sx={styles.divider} />}
            {versionRows
              .filter(r => !r.valueKey)
              .map(row => (
                <ResourceVersionRow
