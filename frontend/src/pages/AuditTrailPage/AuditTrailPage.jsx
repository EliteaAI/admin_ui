...
const AuditTrailPage = memo(() => {
  ...
  const tabEventTypes = auditTab === 'user' ? USER_EVENT_TYPES : SYSTEM_EVENT_TYPES;
  const effectiveEventType = appliedFilters.event_type || tabEventTypes.join(',');
  const eventTypeOptions = auditTab === 'user' ? USER_EVENT_TYPE_OPTIONS : SYSTEM_EVENT_TYPE_OPTIONS;

  // Shared filter params (used by both table and heatmap)
  const dateFromISO =
    appliedFilters.date_from instanceof Date ? appliedFilters.date_from.toISOString() : undefined;
  const dateToISO = appliedFilters.date_to instanceof Date ? appliedFilters.date_to.toISOString() : undefined;
  ...
  const heatmapParams = useMemo(
    ...
    [dateFromISO, dateToISO, debouncedSearch, appliedFilters, traceFilter, refreshKey, effectiveEventType],
  );
