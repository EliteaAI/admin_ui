...
const ReportsPage = memo(() => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (!result?.rows?.length) {
        setError(`No responses for "${selectedSurvey?.name ?? 'survey'}" in the selected period.`);
        return;
      }

      setTableData(result.rows);
    } catch (err) {
      setError(err?.data?.message ?? err?.message ?? 'Failed to generate report');
    }
  ...
  const columns = useMemo(() => {
    ...
    return selectedSurvey.questions.slice().sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
  }, [selectedSurvey]);
  ...
  const groupedRows = useMemo(() => {
    ...
    for (const row of tableData) {
      ...
      entry.answers[row.question_id] = Array.isArray(value) ? value.join(', ') : String(value);
    }
    ...
    <DrawerPage>
      ...
      <Box sx={styles.content}>
        ...
        {!selectedSurveyId && !tableData && (
          ...
          >
            Select a survey and date range, then click Generate to view responses.
          </Typography>
        )}
