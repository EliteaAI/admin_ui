...
import TaskNamesList from './components/TaskNamesList';
import TasksTable from './components/TasksTable';

const COMPLETION_DELAY_MS = 15_000;

const parseTaskName = meta => {
  if (!meta) return '';
  try {
...
const TasksTab = memo(props => {
  const { search = '' } = props;

  const styles = tasksTabStyles();
...
const tasksTabStyles = () => ({
  ...
  descriptionBox: ({ palette }) => ({
    ...
    border: `0.0625rem solid ${palette.border.subtle.default}`,
    flexShrink: 0,
  }),
  descriptionTitle: {
    display: 'block',
