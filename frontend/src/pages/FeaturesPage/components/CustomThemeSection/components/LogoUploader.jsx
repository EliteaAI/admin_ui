...
import {
  ...
  LOGO_MAX_SIZE_BYTES,
  LOGO_MAX_SIZE_LABEL,
} from '@/pages/FeaturesPage/components/CustomThemeSection/constants/customTheme.constants';

const LogoUploader = memo(props => {
  const { logoUrl, onUpload, onDelete, onError, isUploading, isDeleting } = props;

  const styles = logoUploaderStyles();
...
const logoUploaderStyles = () => ({
  ...
  dropZone: ({ palette }) => ({
    ...
    gap: '0.5rem',
    padding: '2rem',
    border: `0.125rem dashed ${palette.border.table}`,
    borderRadius: '0.5rem',
    cursor: 'pointer',
