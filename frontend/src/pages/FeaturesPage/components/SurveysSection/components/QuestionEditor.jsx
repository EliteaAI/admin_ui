...
const getPositionError = (question, allQuestions, index) => {
  ...
  if (allQuestions.some((q, i) => i !== index && (q.position ?? 0) === pos)) return 'Duplicate';
  return undefined;
};

const getSliderError = question => {
  if (question.question_type !== 'slider' || question.options == null) return undefined;
...
const QuestionEditor = memo(props => {
  ...
  const styles = questionEditorStyles();
  ...
  const handleTypeChange = useCallback(
    e => {
      ...
      onChange(index, {
        ...
        options: DEFAULT_OPTIONS[newType] ? structuredClone(DEFAULT_OPTIONS[newType]) : null,
      });
    },
    [index, question, onChange],
  );
    ...
    <Box sx={styles.card}>
      ...
      <Box sx={styles.fieldRow}>
        ...
        <TextField
          ...
          onChange={e => handleField('position', parseInt(e.target.value, 10) || 0)}
          error={!!positionError}
        ...
        <Box sx={styles.optionsSection}>
          ...
          <Box sx={styles.sliderRow}>
            <TextField
              ...
              onChange={e => handleOptionField('min', parseInt(e.target.value, 10) || 0)}
              error={!!sliderError}
            ...
            <TextField
              ...
              onChange={e => handleOptionField('max', parseInt(e.target.value, 10) || 0)}
              error={!!sliderError}
              sx={styles.numberField}
            />
          </Box>
...
const questionEditorStyles = () => ({
  ...
  positionField: {
    ...
    '& .MuiInputBase-input': { fontSize: '0.8125rem' },
    '& .MuiInputLabel-root': { fontSize: '0.8125rem' },
    '& .MuiFormHelperText-root': { fontSize: '0.625rem', margin: '0.125rem 0 0' },
  },
  optionsSection: {
    display: 'flex',
    flexDirection: 'column',
